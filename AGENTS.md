...
| 18 | `initScrambleText` | rAF + `inView()` | `[data-scramble]` elements (the 4 section eyebrows) get a 900ms char-scramble tween (easeInOutQuart) on `inView({ amount: 0.4 })`. Element is restructured into a 2-span container (`.scramble-text__sizer` for width lock, `.scramble-text__live` for the cycle). Char pool is `A-Z` + `0-9` + the target's own special chars. REDUCED snaps to target text with no animation |
